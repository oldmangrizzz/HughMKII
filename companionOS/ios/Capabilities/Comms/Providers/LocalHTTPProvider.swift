...
final class LocalHTTPProvider: LLMProvider {
  ...
  func chat(_ request: ChatRequest, token: String?) async throws -> ChatResponse {
    // Use OpenAI-compatible /v1/chat/completions path (LFM proxied via H.U.G.H. API nginx)
    ...
    var urlRequest = URLRequest(url: base.appendingPathComponent(Constants.lfmChatPath))
